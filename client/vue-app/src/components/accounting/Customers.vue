...
const items = computed(() => {
  const counts = {
    ...
    confirmed: collection.value.filter((item) => item.status === "PM Confirmed").length,
    invoice: collection.value.filter((item) => item.status === "Invoice Received").length,
    expired: collection.value.filter((item) => item.status === "Expired").length,
  };
  return counts;
});
...
async function initialize() {
  ...
  console.log('initialize emitted');
