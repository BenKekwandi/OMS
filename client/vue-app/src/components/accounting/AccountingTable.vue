<template>
  ...
  >
    ...
    <!-- <template v-slot:item.action="{ item }">
      <v-btn @click="showPayments(item)" color="#193a63" v-bind="props" icon="mdi-credit-card" size="small"
        variant="text"></v-btn>
    </template> -->
    <!-- supplier -->
          ...
          <v-row>
            <v-col cols="12" md="6">
              <v-select
                ...
                item-title="company"
                item-value="id"
                :items="company_store.collection"
              ></v-select>
            </v-col>
            <v-col cols="12" sm="6" md="6">
              <DatePicker
...
const dialogInvoice = defineModel("dialogInvoice", { type: Boolean });
...
const headers = ref([
  ...
  { title: "Supplier Shipment Date", key: "" },
  { title: "Tracking #", key: "" },
  { title: "Delivery Date", key: "proposal.delivery_days" },
  { title: "Invoice for Shipment", key: "" },
  { title: "Order Finalization Date", key: "" },
...
const invoicePaymentDeadlineDate = ref(null);

watch(
  () => invoiceDate.value,
  (val) => {
    invoice.value.invoicing_date = format(val, "do MMM yyyy");
  }
);

watch(
  () => invoicePaymentDeadlineDate.value,
  (val) => {
    invoice.value.payment_deadline = format(val, "do MMM yyyy");
  }
);

onMounted(async () => {
  await expenseTypeStorage.fetchItems();
  await company_store.fetchItems();
});
...
const editInvoice = async (item, types) => {
  ...
  invoiceDate.value = new Date(item[invoiceType].invoicing_date);
  invoicePaymentDeadlineDate.value = new Date(
    item[invoiceType].payment_deadline
  );

  proposal_sell_price.value = item.proposal.sll_price;
  dialogInvoice.value = true;
  type.value = types;
};
